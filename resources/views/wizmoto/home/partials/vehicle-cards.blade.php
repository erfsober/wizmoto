@foreach ($advertisements as $advertisement)
    @php
        $image = $advertisement->getMedia('covers')->first();
    @endphp
    <div class="service-block-thirteen">
        <div class="inner-box">
            <div class="image-box">
                <div class="fair-price-overlay">
                    @include('wizmoto.partials.price-evaluation-badge', ['value' => $advertisement->price_evaluation])
                </div>
                <figure class="image">
                    <a href="{{ $image?->getUrl('preview') }}" data-fancybox="gallery-{{ $advertisement->id }}">
                        <img src="{{ $image?->getUrl('vehicle-card') }}" 
                             loading="lazy"
                             alt="{{ $advertisement->title ?? 'Advertisement Image' }}">
                    </a>
                </figure>
            </div>
            <div class="right-box">
                <div class="content-box">
                    <h4 class="title">
                        <a href="{{ route('advertisements.show', $advertisement->id) }}">{{ $advertisement->brand?->localized_name }}{{ ' ' }}{{ $advertisement->vehicleModel?->localized_name }}</a>
                    </h4>
                    <div class="text">{{ $advertisement->version_model }}</div>
                    <div class="inspection-sec">
                        <div class="inspection-box">
                            <span class="icon">
                                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g clip-path="url(#clip0_2880_5319)">
                                        <path d="M18 10.6482C18 12.6227 17.3716 14.497 16.1827 16.0687C15.9482 16.379 15.5071 16.4391 15.1978 16.2052C14.8881 15.971 14.827 15.53 15.0612 15.2203C16.0638 13.895 16.5938 12.3139 16.5938 10.6482C16.5938 6.45117 13.1947 3.05859 9 3.05859C4.8024 3.05859 1.40625 6.45378 1.40625 10.6482C1.40625 12.3139 1.9362 13.895 2.93871 15.2203C3.17299 15.53 3.11188 15.971 2.8022 16.2052C2.49239 16.4395 2.05156 16.3784 1.81714 16.0687C0.628418 14.497 0 12.6227 0 10.6482C0 5.67361 4.02814 1.65234 9 1.65234C13.9746 1.65234 18 5.67636 18 10.6482ZM13.4551 6.41368C13.7296 6.6882 13.7296 7.13342 13.4551 7.40794L11.1632 9.69983C11.3519 10.0477 11.4593 10.4459 11.4593 10.8686C11.4593 12.2248 10.356 13.3279 9 13.3279C7.64388 13.3279 6.54071 12.2248 6.54071 10.8686C6.54071 9.51265 7.64388 8.40935 9 8.40935C9.42284 8.40935 9.82095 8.51674 10.1688 8.70543L12.4607 6.41354C12.7354 6.13902 13.1804 6.13902 13.4551 6.41368ZM10.053 10.8688C10.053 10.2881 9.58063 9.81573 9 9.81573C8.41937 9.81573 7.94696 10.2881 7.94696 10.8688C7.94696 11.4494 8.41937 11.9218 9 11.9218C9.58063 11.9218 10.053 11.4494 10.053 10.8688Z" fill="#050B20"></path>
                                    </g>
                                    <defs>
                                        <clipPath id="clip0_2880_5319">
                                            <rect width="18" height="18" fill="white"></rect>
                                        </clipPath>
                                    </defs>
                                </svg>
                            </span>
                            <div class="info">
                                <span>{{ __('messages.mileage') }}</span>
                                <small>{{ $advertisement->mileage ? number_format($advertisement->mileage) : 'N/A' }}</small>
                            </div>
                        </div>
                        <div class="inspection-box">
                            <span class="icon">
                                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g clip-path="url(#clip0_2880_5323)">
                                        <path d="M10.6875 3.375C10.6875 3.06432 10.4357 2.8125 10.125 2.8125H4.5C4.18932 2.8125 3.9375 3.06432 3.9375 3.375V7.875C3.9375 8.18568 4.18932 8.4375 4.5 8.4375H10.125C10.4357 8.4375 10.6875 8.18568 10.6875 7.875V3.375ZM9.5625 7.3125H5.0625V3.9375H9.5625V7.3125Z" fill="#050B20"></path>
                                        <path d="M17.14 3.98858L14.8967 2.86358C14.6182 2.72405 14.2843 2.8375 14.1453 3.11516C14.0064 3.39311 14.1206 3.73096 14.3986 3.86993L15.2529 4.29669C15.2301 4.35786 15.2005 4.42255 15.2005 4.49177C15.2005 5.22415 15.75 5.84269 16.3125 6.0756V12.9293C16.3125 13.2394 16.0601 13.4918 15.75 13.4918C15.4399 13.4918 15.1875 13.2394 15.1875 12.9293V8.42927C15.1875 7.07115 14.0625 5.93497 12.9375 5.67362V2.24177C12.9375 1.00114 11.955 0 10.7144 0H3.96436C2.72373 0 1.6875 1.00114 1.6875 2.24177V14.8315L0.873422 15.2386C0.682805 15.3339 0.5625 15.5286 0.5625 15.7418V17.4293C0.5625 17.7399 0.841219 18 1.15186 18H13.5269C13.8375 18 14.0625 17.7399 14.0625 17.4293V15.7418C14.0625 15.5286 13.9422 15.3339 13.7516 15.2386L12.9375 14.8315V6.84545C13.5 7.07836 14.0625 7.6969 14.0625 8.42927V12.9293C14.0625 13.8598 14.8194 14.6168 15.75 14.6168C16.6806 14.6168 17.4375 13.8598 17.4375 12.9293V4.49177C17.4375 4.27862 17.3306 4.08389 17.14 3.98858ZM12.9375 16.875H1.6875V16.0895L2.50158 15.6824C2.6922 15.5871 2.8125 15.3924 2.8125 15.1792V2.24174C2.8125 1.6213 3.34389 1.125 3.96436 1.125H10.7144C11.3348 1.125 11.8125 1.6213 11.8125 2.24177V15.1793C11.8125 15.3924 11.9328 15.5872 12.1234 15.6825L12.9375 16.0895V16.875Z" fill="#050B20"></path>
                                    </g>
                                    <defs>
                                        <clipPath id="clip0_2880_5323">
                                            <rect width="18" height="18" fill="white"></rect>
                                        </clipPath>
                                    </defs>
                                </svg>
                            </span>
                            <div class="info">
                                <span>{{ __('messages.fuel_type') }}</span>
                                <small>{{ $advertisement->fuelType?->localized_name ?? 'N/A' }}</small>
                            </div>
                        </div>
                        <div class="inspection-box">
                            <span class="icon">
                                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M3 1.6875C1.8615 1.6875 0.9375 2.6115 0.9375 3.75C0.9375 4.8885 1.8615 5.8125 3 5.8125C4.1385 5.8125 5.0625 4.8885 5.0625 3.75C5.0625 2.6115 4.1385 1.6875 3 1.6875ZM3 2.8125C3.5175 2.8125 3.9375 3.2325 3.9375 3.75C3.9375 4.2675 3.5175 4.6875 3 4.6875C2.4825 4.6875 2.0625 4.2675 2.0625 3.75C2.0625 3.2325 2.4825 2.8125 3 2.8125Z" fill="#050B20"></path>
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M3 12.1875C1.8615 12.1875 0.9375 13.1115 0.9375 14.25C0.9375 15.3885 1.8615 16.3125 3 16.3125C4.1385 16.3125 5.0625 15.3885 5.0625 14.25C5.0625 13.1115 4.1385 12.1875 3 12.1875ZM3 13.3125C3.5175 13.3125 3.9375 13.7325 3.9375 14.25C3.9375 14.7675 3.5175 15.1875 3 15.1875C2.4825 15.1875 2.0625 14.7675 2.0625 14.25C2.0625 13.7325 2.4825 13.3125 3 13.3125Z" fill="#050B20"></path>
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M9 1.6875C7.8615 1.6875 6.9375 2.6115 6.9375 3.75C6.9375 4.8885 7.8615 5.8125 9 5.8125C10.1385 5.8125 11.0625 4.8885 11.0625 3.75C11.0625 2.6115 10.1385 1.6875 9 1.6875ZM9 2.8125C9.5175 2.8125 9.9375 3.2325 9.9375 3.75C9.9375 4.2675 9.5175 4.6875 9 4.6875C8.4825 4.6875 8.0625 4.2675 8.0625 3.75C8.0625 3.2325 8.4825 2.8125 9 2.8125Z" fill="#050B20"></path>
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M9 12.1875C7.8615 12.1875 6.9375 13.1115 6.9375 14.25C6.9375 15.3885 7.8615 16.3125 9 16.3125C10.1385 16.3125 11.0625 15.3885 11.0625 14.25C11.0625 13.1115 10.1385 12.1875 9 12.1875ZM9 13.3125C9.5175 13.3125 9.9375 13.7325 9.9375 14.25C9.9375 14.7675 9.5175 15.1875 9 15.1875C8.4825 15.1875 8.0625 14.7675 8.0625 14.25C8.0625 13.7325 8.4825 13.3125 9 13.3125Z" fill="#050B20"></path>
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M15 1.6875C13.8615 1.6875 12.9375 2.6115 12.9375 3.75C12.9375 4.8885 13.8615 5.8125 15 5.8125C16.1385 5.8125 17.0625 4.8885 17.0625 3.75C17.0625 2.6115 16.1385 1.6875 15 1.6875ZM15 2.8125C15.5175 2.8125 15.9375 3.2325 15.9375 3.75C15.9375 4.2675 15.5175 4.6875 15 4.6875C14.4825 4.6875 14.0625 4.2675 14.0625 3.75C14.0625 3.2325 14.4825 2.8125 15 2.8125Z" fill="#050B20"></path>
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M15 12.1875C13.8615 12.1875 12.9375 13.1115 12.9375 14.25C12.9375 15.3885 13.8615 16.3125 15 16.3125C16.1385 16.3125 17.0625 15.3885 17.0625 14.25C17.0625 13.1115 16.1385 12.1875 15 12.1875ZM15 13.3125C15.5175 13.3125 15.9375 13.7325 15.9375 14.25C15.9375 14.7675 15.5175 15.1875 15 15.1875C14.4825 15.1875 14.0625 14.7675 14.0625 14.25C14.0625 13.7325 14.4825 13.3125 15 13.3125Z" fill="#050B20"></path>
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M2.4375 5.25V12.75C2.4375 13.0605 2.6895 13.3125 3 13.3125C3.3105 13.3125 3.5625 13.0605 3.5625 12.75V5.25C3.5625 4.9395 3.3105 4.6875 3 4.6875C2.6895 4.6875 2.4375 4.9395 2.4375 5.25Z" fill="#050B20"></path>
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M8.4375 5.25V12.75C8.4375 13.0605 8.6895 13.3125 9 13.3125C9.3105 13.3125 9.5625 13.0605 9.5625 12.75V5.25C9.5625 4.9395 9.3105 4.6875 9 4.6875C8.6895 4.6875 8.4375 4.9395 8.4375 5.25Z" fill="#050B20"></path>
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M14.4375 5.25V8.25C14.4375 8.3535 14.3535 8.4375 14.25 8.4375H3C2.6895 8.4375 2.4375 8.6895 2.4375 9C2.4375 9.3105 2.6895 9.5625 3 9.5625H14.25C14.9753 9.5625 15.5625 8.9745 15.5625 8.25C15.5625 7.15575 15.5625 5.25 15.5625 5.25C15.5625 4.9395 15.3105 4.6875 15 4.6875C14.6895 4.6875 14.4375 4.9395 14.4375 5.25Z" fill="#050B20"></path>
                                </svg>
                            </span>
                            <div class="info">
                                <span>{{ __('messages.transmission') }}</span>
                                <small>{{ $advertisement->motor_change ?? 'N/A' }}</small>
                            </div>
                        </div>
                    </div>
                    <ul class="ul-cotent">
                        @forelse($advertisement->equipments->take(2) as $equipment)
                            <li><a href="#">{{ Str::limit($equipment->localized_name, 20) }}</a></li>
                        @empty
                            <li><a href="#">{{ __('messages.standard_features') }}</a></li>
                        @endforelse
                    </ul>
                </div>
                <div class="content-box-two" style="display: flex;flex-direction: column;justify-content: center;align-items: baseline;">
                    <h4 class="title">€{{ $advertisement->final_price }}</h4>
                    <a href="{{ route('advertisements.show', $advertisement->id) }}" class="button">{{ __('messages.view_details') }}<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
                            <g clip-path="url(#clip0_989_6940)">
                                <path d="M13.6106 0H5.05509C4.84013 0 4.66619 0.173943 4.66619 0.388901C4.66619 0.603859 4.84013 0.777802 5.05509 0.777802H12.6719L0.113453 13.3362C-0.0384687 13.4881 -0.0384687 13.7342 0.113453 13.8861C0.189396 13.962 0.288927 14 0.388422 14C0.487917 14 0.587411 13.962 0.663391 13.8861L13.2218 1.3277V8.94447C13.2218 9.15943 13.3957 9.33337 13.6107 9.33337C13.8256 9.33337 13.9996 9.15943 13.9996 8.94447V0.388901C13.9995 0.173943 13.8256 0 13.6106 0Z" fill="#405FF2"></path>
                            </g>
                            <defs>
                                <clipPath id="clip0_989_6940">
                                    <rect width="14" height="14" fill="white"></rect>
                                </clipPath>
                            </defs>
                        </svg>
                    </a>
                </div>
            </div>
        </div>
    </div>
@endforeach

@if ($advertisements->hasPages())
    <div class="pagination-sec">
        <nav aria-label="Page navigation example">
            <ul class="pagination pagination-desktop">
                <!-- Previous Button -->
                <li class="page-item {{ $advertisements->onFirstPage() ? 'disabled' : '' }}">
                    <a class="page-link" href="{{ $advertisements->previousPageUrl() }}" aria-label="Previous">
                        <span aria-hidden="true">
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M2.57983 5.99989C2.57983 5.7849 2.66192 5.56987 2.82573 5.4059L7.98559 0.24617C8.31382 -0.0820565 8.84598 -0.0820565 9.17408 0.24617C9.50217 0.574263 9.50217 1.10632 9.17408 1.43457L4.60841 5.99989L9.17376 10.5654C9.50185 10.8935 9.50185 11.4256 9.17376 11.7537C8.84566 12.0821 8.31366 12.0821 7.98544 11.7537L2.82555 6.59404C2.66176 6.42999 2.57983 6.21495 2.57983 5.99989Z"
                                    fill="#050B20" />
                            </svg>
                        </span>
                        <span class="sr-only">Previous</span>
                    </a>
                </li>

                <!-- First Page -->
                @if ($advertisements->currentPage() > 3)
                    <li class="page-item">
                        <a class="page-link" href="{{ $advertisements->url(1) }}">1</a>
                    </li>
                    @if ($advertisements->currentPage() > 4)
                        <li class="page-item disabled">
                            <span class="page-link">...</span>
                        </li>
                    @endif
                @endif

                <!-- Page Numbers -->
                @php
                    $start = max(1, $advertisements->currentPage() - 2);
                    $end = min($advertisements->lastPage(), $advertisements->currentPage() + 2);
                @endphp

                @for ($i = $start; $i <= $end; $i++)
                    <li class="page-item {{ $i == $advertisements->currentPage() ? 'active' : '' }}">
                        <a class="page-link" href="{{ $advertisements->url($i) }}">{{ $i }}</a>
                    </li>
                @endfor

                <!-- Last Page -->
                @if ($advertisements->currentPage() < $advertisements->lastPage() - 2)
                    @if ($advertisements->currentPage() < $advertisements->lastPage() - 3)
                        <li class="page-item disabled">
                            <span class="page-link">...</span>
                        </li>
                    @endif
                    <li class="page-item">
                        <a class="page-link"
                            href="{{ $advertisements->url($advertisements->lastPage()) }}">{{ $advertisements->lastPage() }}</a>
                    </li>
                @endif

                <!-- Next Button -->
                <li class="page-item {{ !$advertisements->hasMorePages() ? 'disabled' : '' }}">
                    <a class="page-link" href="{{ $advertisements->nextPageUrl() }}" aria-label="Next">
                        <span aria-hidden="true">
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_2880_6407)">
                                    <path
                                        d="M9.42017 6.00011C9.42017 6.2151 9.33808 6.43013 9.17427 6.5941L4.01441 11.7538C3.68618 12.0821 3.15402 12.0821 2.82592 11.7538C2.49783 11.4257 2.49783 10.8937 2.82592 10.5654L7.39159 6.00011L2.82624 1.43461C2.49815 1.10652 2.49815 0.574382 2.82624 0.246315C3.15434 -0.0820709 3.68634 -0.0820709 4.01457 0.246315L9.17446 5.40596C9.33824 5.57001 9.42017 5.78505 9.42017 6.00011Z"
                                        fill="#050B20" />
                                </g>
                                <defs>
                                    <clipPath id="clip0_2880_6407">
                                        <rect width="12" height="12" fill="white"
                                            transform="translate(12 12) rotate(-180)" />
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                        <span class="sr-only">Next</span>
                    </a>
                </li>
            </ul>

            <!-- Mobile Compact Pagination -->
            <ul class="pagination pagination-mobile">
                <!-- Previous Button -->
                <li class="page-item {{ $advertisements->onFirstPage() ? 'disabled' : '' }}">
                    <a class="page-link" href="{{ $advertisements->previousPageUrl() }}" aria-label="Previous">
                        <span aria-hidden="true">
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M2.57983 5.99989C2.57983 5.7849 2.66192 5.56987 2.82573 5.4059L7.98559 0.24617C8.31382 -0.0820565 8.84598 -0.0820565 9.17408 0.24617C9.50217 0.574263 9.50217 1.10632 9.17408 1.43457L4.60841 5.99989L9.17376 10.5654C9.50185 10.8935 9.50185 11.4256 9.17376 11.7537C8.84566 12.0821 8.31366 12.0821 7.98544 11.7537L2.82555 6.59404C2.66176 6.42999 2.57983 6.21495 2.57983 5.99989Z"
                                    fill="#050B20" />
                            </svg>
                        </span>
                        <span class="sr-only">Previous</span>
                    </a>
                </li>

                <!-- First Page -->
                @if ($advertisements->currentPage() > 2)
                    <li class="page-item">
                        <a class="page-link" href="{{ $advertisements->url(1) }}">1</a>
                    </li>
                    @if ($advertisements->currentPage() > 3)
                        <li class="page-item disabled"><span class="page-link">...</span></li>
                    @endif
                @endif

                <!-- Current - 1 -->
                @if ($advertisements->currentPage() - 1 >= 1)
                    <li class="page-item {{ $advertisements->currentPage() - 1 == $advertisements->currentPage() ? 'active' : '' }}">
                        <a class="page-link" href="{{ $advertisements->url($advertisements->currentPage() - 1) }}">
                            {{ $advertisements->currentPage() - 1 }}
                        </a>
                    </li>
                @endif

                <!-- Current -->
                <li class="page-item active">
                    <a class="page-link" href="{{ $advertisements->url($advertisements->currentPage()) }}">
                        {{ $advertisements->currentPage() }}
                    </a>
                </li>

                <!-- Current + 1 -->
                @if ($advertisements->currentPage() + 1 <= $advertisements->lastPage())
                    <li class="page-item {{ $advertisements->currentPage() + 1 == $advertisements->currentPage() ? 'active' : '' }}">
                        <a class="page-link" href="{{ $advertisements->url($advertisements->currentPage() + 1) }}">
                            {{ $advertisements->currentPage() + 1 }}
                        </a>
                    </li>
                @endif

                <!-- Last Page -->
                @if ($advertisements->currentPage() < $advertisements->lastPage() - 1)
                    @if ($advertisements->currentPage() < $advertisements->lastPage() - 2)
                        <li class="page-item disabled"><span class="page-link">...</span></li>
                    @endif
                    <li class="page-item">
                        <a class="page-link" href="{{ $advertisements->url($advertisements->lastPage()) }}">
                            {{ $advertisements->lastPage() }}
                        </a>
                    </li>
                @endif

                <!-- Next Button -->
                <li class="page-item {{ !$advertisements->hasMorePages() ? 'disabled' : '' }}">
                    <a class="page-link" href="{{ $advertisements->nextPageUrl() }}" aria-label="Next">
                        <span aria-hidden="true">
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_2880_6407)">
                                    <path
                                        d="M9.42017 6.00011C9.42017 6.2151 9.33808 6.43013 9.17427 6.5941L4.01441 11.7538C3.68618 12.0821 3.15402 12.0821 2.82592 11.7538C2.49783 11.4257 2.49783 10.8937 2.82592 10.5654L7.39159 6.00011L2.82624 1.43461C2.49815 1.10652 2.49815 0.574382 2.82624 0.246315C3.15434 -0.0820709 3.68634 -0.0820709 4.01457 0.246315L9.17446 5.40596C9.33824 5.57001 9.42017 5.78505 9.42017 6.00011Z"
                                        fill="#050B20" />
                                </g>
                                <defs>
                                    <clipPath id="clip0_2880_6407">
                                        <rect width="12" height="12" fill="white"
                                            transform="translate(12 12) rotate(-180)" />
                                    </clipPath>
                                </defs>
                            </svg>
                        </span>
                        <span class="sr-only">Next</span>
                    </a>
                </li>
            </ul>

            <!-- Results Info -->
            <div class="pagination-info">
                <div class="text">Showing {{ $advertisements->firstItem() ?? 0 }} to
                    {{ $advertisements->lastItem() ?? 0 }} of {{ $advertisements->total() ?? 0 }} vehicles</div>
                <div class="page-info">Page {{ $advertisements->currentPage() }} of {{ $advertisements->lastPage() }}
                </div>
            </div>
        </nav>
    </div>
@endif
